        ...
        for (const auto& ir_func : ir_module_.funcs()) {
            ...
            functions_.push_back(
                llvm::Function::Create(
                    ...
                    llvm::StringRef(
                        ir_func.name().data(), ir_func.name().size()
                    ),
                    *llvm_module_
                )
            );
...
private:
    ...
    llvm::Value* get_value(
        ...
        refanal::ir::InstRef ref,
        types::Specifier spec
    );
    llvm::Value* get_value_from_ptr(
        llvm::Value* ptr, types::SpecType ptr_type, types::SpecType value_type
    );
    types::SpecType get_item_type(
        const refanal::ir::Func& ir_func, refanal::ir::InstRef ref
    );
...
llvm::Type* Generator::get_base_type(types::TypeId type_id) {
    ...
    return t.data.visit(
        ...
        [&](const types::Type::Struct& s) -> llvm::Type* {
            if (auto* st = llvm::StructType::getTypeByName(context_, s.name)) {
                return st;
            }
...
llvm::Type* Generator::get_rep_type(types::SpecType st) {
...
llvm::Value* Generator::get_value(
    ...
    types::SpecType result_type = {.type = inst.type.type, .specifier = spec};
    ...
    if (is_ref(result_type)) {
        if (is_ref(inst.type)) {
            ...
            llvm::IRBuilder<> eb(
                &llvm_func->getEntryBlock(), llvm_func->getEntryBlock().begin()
            );
            llvm::Value* temp =
        ...
        if (is_ref(inst.type)) {
            return builder_.CreateLoad(get_base_type(result_type.type), value);
        } else {
            return value;
        }
    }
...
llvm::Value* Generator::get_value_from_ptr(
    llvm::Value* ptr, types::SpecType ptr_type, types::SpecType value_type
) {
...
types::SpecType Generator::get_item_type(
    const refanal::ir::Func& ir_func, refanal::ir::InstRef ref
) {
    auto& type = ir_module_.types().get(ir_func.inst(ref).type.type);
...
void Generator::generate_func(
    ...
    for (auto i : ir_func.blocks().indices()) {
        blocks_.push_back(
            llvm::BasicBlock::Create(
                context_, "block" + std::to_string(i.index), llvm_func
            )
        );
    }
    inst_values_.clear();
    ...
    for (auto i : ir_func.params().indices()) {
        ...
        args.push_back(builder_.CreateAlloca(
            get_rep_type(param.type), nullptr, std::string(param.name) + ".arg"
        ));
    }
    for (auto i : ir_func.params().indices()) {
        builder_.CreateStore(llvm_func->getArg(i.index), args[i.index]);
    }
    ...
    for (auto b_idx : ir_func.blocks().indices()) {
        ...
        for (auto inst_ref : ir_func.block(b_idx).insts) {
            ...
            inst_values_[inst_ref] = inst.data.visit(
                [&](const refanal::ir::Inst::Const& c) -> llvm::Value* {
                    return c.value.visit(
                        [&](bool b) -> llvm::Value* {
                            return llvm::ConstantInt::get(
                                llvm::Type::getInt1Ty(context_), b
                            );
                        },
                        ...
                        [&](char32_t ch) -> llvm::Value* {
                            return llvm::ConstantInt::get(
                                llvm::Type::getInt32Ty(context_), ch
                            );
                        },
                ...
                [&](const refanal::ir::Inst::LoadParam& lp) -> llvm::Value* {
                    ...
                    return get_value_from_ptr(
                        args[lp.param.index], param.type, inst.type
                    );
                },
                [&](const refanal::ir::Inst::Store& s) -> llvm::Value* {
                    ...
                    auto value = get_value(
                        ir_func, llvm_func, s.value, dest_info.type.specifier
                    );
                    return builder_.CreateStore(value, dest_storage);
                },
                [&](const refanal::ir::Inst::Binary& b) -> llvm::Value* {
                    llvm::Value* l = get_value(
                        ir_func, llvm_func, b.left, types::Specifier::Val
                    );
                    llvm::Value* r = get_value(
                        ir_func, llvm_func, b.right, types::Specifier::Val
                    );
                    bool f = l->getType()->isFloatingPointTy();
                    const auto& left_type =
                        ir_module_.types().get(ir_func.inst(b.left).type.type);
                    bool is_signed = true;
                ...
                [&](const refanal::ir::Inst::Comparison& c) -> llvm::Value* {
                    llvm::Value* l = get_value(
                        ir_func, llvm_func, c.left, types::Specifier::Val
                    );
                    llvm::Value* r = get_value(
                        ir_func, llvm_func, c.right, types::Specifier::Val
                    );
                    bool f = l->getType()->isFloatingPointTy();
                    const auto& left_type =
                        ir_module_.types().get(ir_func.inst(c.left).type.type);
                    bool is_signed = true;
                    if (auto* i = left_type.data.get_if<types::Type::Int>()) {
                        is_signed = i->is_signed;
                    }
                ...
                [&](const refanal::ir::Inst::Call& c) -> llvm::Value* {
                    llvm::Value* callee = get_value(
                        ir_func, llvm_func, c.value, types::Specifier::Val
                    );

                    const auto& t =
                        ir_module_.types().get(ir_func.inst(c.value).type.type);
                    ...
                    if (auto* f = t.data.get_if<types::Type::Func>()) {
                        ...
                        func_type_info = ir_module_.types()
                                             .get(p->type.type)
                    ...
                    for (size_t i = 0; i < c.args.count; i++) {
                        args.push_back(get_value(
                            ...
                            llvm_func,
                            arg_refs[i],
                            func_type_info->params[i].specifier
                        ));
                    }
                ...
                [&](const refanal::ir::Inst::Return& r) -> llvm::Value* {
                    if (r.value) {
                        return builder_.CreateRet(get_value(
                            ir_func,
                            llvm_func,
                            *r.value,
                            ir_func.return_type().specifier
                        ));
                    } else {
                ...
                [&](const refanal::ir::Inst::Cast& cast) -> llvm::Value* {
                    ...
                    if (from_type_id == to_type_id) {
                        return get_value(
                            ir_func, llvm_func, cast.value, inst.type.specifier
                        );
                    }
                    ...
                    llvm::Value* val = get_value(
                        ir_func, llvm_func, cast.value, types::Specifier::Val
                    );
                        ...
                        [&](const types::Type::Bool&) -> llvm::Value* {
                            ...
                            if (to_type.data.is<types::Type::Float>())
                                return builder_.CreateFCmpONE(
                                    val,
                                    llvm::ConstantFP::getZero(val->getType())
                                );
                    ...
                    for (uint32_t i = 0; i < args.size(); ++i) {
                        sv = builder_.CreateInsertValue(
                            ...
                            get_value(
                                ...
                                llvm_func,
                                args[i],
                                struct_def.fields[i].type.specifier
                            ),
                            {i}
                        );
                    }
                ...
                [&](const refanal::ir::Inst::GetField& gf) -> llvm::Value* {
                    ...
                    const auto& struct_type =
                        ir_module_.types()
                            .get(base_info.type.type)
                            .data.get<types::Type::Struct>();
                    if (is_ref(base_info.type)) {
                        llvm::Value* ptr = inst_values_[gf.value];
                        llvm::Type* base_type =
                            get_base_type(base_info.type.type);
                        llvm::Value* field_ptr =
                            builder_.CreateStructGEP(base_type, ptr, gf.index);
                        return get_value_from_ptr(
                            field_ptr,
                            struct_type.fields[gf.index].type,
                            inst.type
                        );
                    } else {
                        llvm::Value* val = get_value(
                            ir_func, llvm_func, gf.value, types::Specifier::Val
                        );
                ...
                [&](const refanal::ir::Inst::AddressOf& ao) -> llvm::Value* {
                    return inst_values_[ao.value];
                },
                [&](const refanal::ir::Inst::Deref& d) -> llvm::Value* {
                    return get_value(
                        ir_func, llvm_func, d.value, types::Specifier::Val
                    );
                },
                [&](const refanal::ir::Inst::GetItem& gi) -> llvm::Value* {
                    llvm::Value* idx = get_value(
                        ir_func, llvm_func, gi.index, types::Specifier::Val
                    );
                    auto item_type = get_item_type(ir_func, gi.value);
                    const auto& base_info = ir_func.inst(gi.value);
                    llvm::Type* base_llvm_type =
                        get_base_type(base_info.type.type);

                    if (base_llvm_type->isArrayTy()) {
                        llvm::Value* array_ptr = get_value(
                            ir_func, llvm_func, gi.value, types::Specifier::Var
                        );
                        llvm::Value* item_ptr = builder_.CreateGEP(
                            base_llvm_type,
                            array_ptr,
                            {builder_.getInt32(0), idx}
                        );
                        return get_value_from_ptr(
                            item_ptr, item_type, inst.type
                        );
                    } else {
                        llvm::Value* ptr = get_value(
                            ir_func, llvm_func, gi.value, types::Specifier::Val
                        );
                        llvm::Value* item_ptr = builder_.CreateGEP(
                ...
                [&](const refanal::ir::Inst::SetItem& si) -> llvm::Value* {
                    llvm::Value* idx = get_value(
                        ir_func, llvm_func, si.index, types::Specifier::Val
                    );
                    auto item_type = get_item_type(ir_func, si.var);
                    llvm::Value* v = get_value(
                        ir_func, llvm_func, si.value, item_type.specifier
                    );
                    ...
                    if (base_llvm_type->isArrayTy()) {
                        llvm::Value* array_ptr = get_value(
                            ir_func, llvm_func, si.var, types::Specifier::Var
                        );
                        llvm::Value* item_ptr = builder_.CreateGEP(
                            base_llvm_type,
                            array_ptr,
                        ...
                        llvm::Value* ptr = get_value(
                            ir_func, llvm_func, si.var, types::Specifier::Val
                        );
                        llvm::Value* item_ptr = builder_.CreateGEP(
                            get_base_type(item_type.type), ptr, idx
                        );
                ...
                [&](const refanal::ir::Inst::Unary& u) -> llvm::Value* {
                    llvm::Value* v = get_value(
                        ir_func, llvm_func, u.value, types::Specifier::Val
                    );
                    switch (u.op) {
                ...
                [&](const refanal::ir::Inst::Array& arr) -> llvm::Value* {
                    ...
                    auto item_spec = ir_module_.types()
                                         .get(inst.type.type)
                    ...
                    for (uint32_t k = 0; k < items.size(); ++k) {
                        av = builder_.CreateInsertValue(
                            av,
                            get_value(ir_func, llvm_func, items[k], item_spec),
                            {k}
                        );
                    }
                    return av;
