...
#include "semanal/type_analyze.h"
...
[[nodiscard]] ir::AFunc TypeAnalyzer::get_types() {
...
const ir::Inst& TypeAnalyzer::get_inst(ir::InstRef ref) const {
    return package_->package_->inst(ref);
}

const ir::Comparator& TypeAnalyzer::get_comparator(
    ir::ComparatorRef ref
) const {
    return package_->package_->comparator(ref);
}
std::span<const ir::InstRef> TypeAnalyzer::get_inst_refs(
    ir::InstRefs refs
) const {
    return package_->package_->inst_refs(refs);
}
std::span<const ir::CallArg> TypeAnalyzer::get_call_args(
    ir::CallArgs args
) const {
...
void TypeAnalyzer::require_var(
    ...
    const auto& inst_ = get_inst(ref);
    bool possible = inst_.data.visit(
        [&](const ir::Inst::LoadVar& data) {
            require_var(data.var, loc, context);
            return true;
        },
        [&](const ir::Inst::VarDecl&) {
            if (tv.locked_spec && tv.type->specifier == types::Specifier::Let) {
                error(
                    loc,
                    std::format(
                ...
                if (auto st = type.data.get_if<types::Type::Struct>()) {
                    for (const auto& field : st->fields) {
                        if (field.name == data.name) {
                            if (field.type.specifier == types::Specifier::Let) {
                                error(
                                    loc,
                                    std::format(
                            ...
                            if (field.type.specifier == types::Specifier::Val) {
...
void TypeAnalyzer::propagate_range(ir::Block range) {
    while (current_inst_ <= range.end.index) {
        propagate_inst({current_inst_++});
    }
}

void TypeAnalyzer::propagate_inst(ir::InstRef ref) {
    const auto& inst = get_inst(ref);
    inst.data.visit(
        ...
        [&](const ir::Inst::If& data) { handle_if(ref, data, inst.location); },
        [&](const ir::Inst::Break&) {
        ...
        [&](const ir::Inst::As& data) { handle_as(ref, data, inst.location); },
        [&](const auto&) {
...
void TypeAnalyzer::handle_const(
...
void TypeAnalyzer::handle_load_var(
...
void TypeAnalyzer::handle_binary(
    ...
    if (left && right) {
        ...
        if (unified) {
            ...
            add_type(
                ref, {.type = *unified, .specifier = types::Specifier::Val}, loc
            );
        }
    }
}

void TypeAnalyzer::handle_logical(
    ...
    if (right_tp && !can_convert(right_tp->type, types::Bool, *type_pool_)) {
        error(
            loc,
            std::format(
                "Type mismatch: cannot convert '{}' to Bool",
...
void TypeAnalyzer::handle_comparison(
    ...
    for (auto comp_ref : data.comparators) {
        const auto& comparator = get_comparator(comp_ref);
...
void TypeAnalyzer::handle_call(
    ...
    if (func_tp.has_value()) {
        ...
        if (auto ft = type.data.get_if<types::Type::Func>()) {
            ...
            if (auto id = get_inst(data.value)
...
void TypeAnalyzer::handle_if(
    ir::InstRef ref, const ir::Inst::If& data, Location loc
) {
...
void TypeAnalyzer::handle_return(
...
void TypeAnalyzer::handle_address_of(
...
void TypeAnalyzer::handle_deref(
...
void TypeAnalyzer::handle_get_item(
...
void TypeAnalyzer::handle_set_item(
...
void TypeAnalyzer::handle_get_attr(
...
void TypeAnalyzer::handle_set_attr(
...
void TypeAnalyzer::handle_array(
    ir::InstRef ref, const ir::Inst::Array& data, Location loc
) {
    auto items = get_inst_refs(data.items);
    if (items.empty()) {
        return;
    }
...
void TypeAnalyzer::check_func(
    ...
    auto args = get_inst_refs(data.args);
    for (size_t i = 0; i < args.size(); ++i) {
        auto arg_tp = type_vars_[args[i]].type;
        if (arg_tp &&
            !can_convert(arg_tp->type, ft.params[i].type.type, *type_pool_)) {
            error(
                inst.location,
                std::format(
    ...
    auto named_args = get_call_args(data.named_args);
...
void TypeAnalyzer::check_struct(
    ...
    for (size_t i = 0; i < args.size(); ++i) {
        auto arg_tp = type_vars_[args[i]].type;
        if (arg_tp &&
            !can_convert(arg_tp->type, type.fields[i].type.type, *type_pool_)) {
            error(
                inst.location,
                std::format(
                    "Type mismatch: cannot convert "
    ...
    for (const auto& named_arg : named_args) {
        auto param = [&] -> std::optional<types::Type::StructField> {
            ...
            error(
                inst.location,
                std::format("struct has no field named '{}'", named_arg.name)
            );
            return std::nullopt;
        }();
        auto arg_tp = type_vars_[named_arg.value].type;
        ...
        if (param && param->type.specifier == types::Specifier::Var) {
            require_var(named_arg.value, inst.location, "named struct field");
        }
    }
}

void TypeAnalyzer::add_type(
    ir::InstRef inst, types::SpecType type, Location loc
) {
    auto& tv = type_vars_[inst];
    bool previously_defined = tv.defined();
    auto previous_type = tv.get();
    auto tp = tv.add_type(
        type, loc, *func_->source, *type_pool_, *package_->err_handler_
    );
...
void TypeAnalyzer::copy_type(ir::InstRef src, ir::InstRef dest, Location loc) {
...
void TypeAnalyzer::lock_type(
    ir::InstRef inst, types::TypeId type, Location loc
) {
    auto& tv = type_vars_[inst];
    if (tv.locked) return;
    tv.lock(type, loc, *func_->source, *type_pool_, *package_->err_handler_);
    changed_ = true;
}
